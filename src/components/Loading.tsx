    ...
    <div className="min-h-screen bg-mesh flex flex-col items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-md">
        ...
        <div className="relative mb-12">
          ...
          <div className="relative w-32 h-32 rounded-full bg-brand flex items-center justify-center animate-spin-slow shadow-glow">
            <div className="w-28 h-28 rounded-full bg-background flex items-center justify-center">
              <Sparkles className="w-12 h-12 text-brand" style={{ background: "var(--gradient-brand)", WebkitBackgroundClip: "text", color: "transparent" } as React.CSSProperties} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3 w-full">
          {steps.map((step, i) => (
            <div
              key={step}
              className={`flex items-center gap-3 justify-center transition-smooth ${
                i <= stepIdx ? "opacity-100" : "opacity-30"
              }`}
            >
              <div className={`w-2 h-2 rounded-full ${i <= stepIdx ? "bg-brand" : "bg-muted"}`} />
