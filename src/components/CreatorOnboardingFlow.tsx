...
export default function CreatorOnboardingFlow({ onBack, onContinue }: Props) {
  ...
  return (
    <div className="relative h-screen overflow-hidden" dir="rtl">
      <div
        key={step}
        className="h-full"
        style={{ animation: `${dir === "forward" ? "cfSlideIn" : "cfSlideBack"} 0.32s cubic-bezier(0.4,0,0.2,1) both` }}
      >
        {step === "benefits" && (
          <BenefitsStep onBack={onBack} onContinue={() => go("how")} />
        )}
        {step === "how" && (
          <HowItWorksStep onBack={() => go("benefits", "back")} onContinue={onContinue} />
        )}
      </div>
      <style>{`
        @keyframes cfSlideIn   { from { transform: translateX(48px);  opacity: 0 } to { transform: translateX(0); opacity: 1 } }
        @keyframes cfSlideBack { from { transform: translateX(-48px); opacity: 0 } to { transform: translateX(0); opacity: 1 } }
      `}</style>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 pt-14 pb-5 px-6 text-center">
        <h2 className="text-2xl font-black text-white mb-2 leading-tight">למה Matchly ליוצרי תוכן?</h2>
