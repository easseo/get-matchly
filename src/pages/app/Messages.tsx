...
export default function Messages() {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden grid md:grid-cols-[300px_1fr] min-h-[580px]">

        {/* Conversations sidebar */}
        <aside className="border-l border-gray-100 bg-gray-50/50 flex flex-col">
          {/* Search */}
          <div className="p-3 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-gray-200 rounded-xl pr-9 pl-3 py-2 text-xs font-medium focus:outline-none focus:ring-1 focus:ring-primary/30"
              />
            </div>
          </div>
            ...
            {filteredConversations.map((c) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-right p-3.5 border-b border-gray-100 flex items-center gap-3 transition-colors ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className={`text-sm truncate ${badge > 0 ? "font-extrabold text-gray-900" : "font-bold text-gray-700"}`}>
                        {c.creatorName}
                      </span>
                      {last && <span className="text-[10px] text-gray-400 shrink-0 mr-1">{last.time}</span>}
                    </div>
                    <div className="flex items-center gap-1">
                      {last?.side === "right" && <CheckCheck className="w-3 h-3 text-primary shrink-0" />}
                      <span className={`text-[11px] truncate ${badge > 0 ? "text-gray-700 font-semibold" : "text-gray-400"}`}>
                        {last?.text ?? c.message}
                      </span>
                    </div>
                  </div>
            ...
            <>
              ...
              <div className="p-4 border-b border-gray-100 flex items-center gap-3 bg-white">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-gray-400">{active.campaignTitle}</div>
                </div>
              ...
              <div className="p-3 border-t border-gray-100 bg-white flex gap-2 items-end">
                <input
                  ...
                  className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none"
                  placeholder="כתבו הודעה..."
                />
                <button
                  onClick={handleSend}
                  disabled={!input.trim()}
                  className="p-3 rounded-2xl text-white disabled:opacity-40 transition-opacity shrink-0"
...
function Bubble({
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[72%]">
          <div className="px-4 py-2.5 rounded-2xl rounded-tr-sm text-sm text-white shadow-sm" style={{ background: "var(--gradient-brand)" }}>
            {children}
          </div>
  ...
  return (
    <div className="flex gap-2 items-end">
      <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${gradient} shrink-0`} />
      <div className="max-w-[72%]">
        <div className="px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm bg-white border border-gray-100 shadow-sm text-gray-700">
          {children}
        </div>
