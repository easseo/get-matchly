...
type ConvRow = {
  ...
  profiles: { full_name: string } | null; // advertiser
};
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("conversations")
      .select("id, campaign_id, creator_id, advertiser_id, campaigns(title), profiles!conversations_advertiser_id_fkey(full_name)")
  ...
  if (convs.length === 0) return (
    <div className="flex-1 flex items-center justify-center p-8 text-center" dir="rtl">
      <div>
        ...
        <p className="text-sm text-gray-400">שיחות נפתחות לאחר שהמפרסם מאשר את הצעתך</p>
      </div>
    </div>
  );
          ...
          {filtered.map((c) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0 text-right">
                <div className="font-bold text-sm text-gray-900 truncate">{c.profiles?.full_name ?? "מפרסם"}</div>
      ...
      {activeConv ? (
        <div className="flex-1 flex flex-col min-w-0 bg-gray-50">
          ...
          <div className="bg-white border-b border-gray-100 px-5 py-3.5 flex items-center gap-3 shrink-0">
            ...
            <div>
              <div className="font-bold text-gray-900 text-sm">{activeConv.profiles?.full_name ?? "מפרסם"}</div>
